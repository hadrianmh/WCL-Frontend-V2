  ...
  <script>
    $(document).ready(function()
    {
      ...
      var tabel = $('#tablenya').DataTable({
        ...
        ordering  : false,
        ajax: {
          url: "<?php echo base_url('index.php/action/all/invoice_done'); ?>",
          type: "POST",
          data: function ( data ) {
      ...
      $(document).on('click', '.PrintView', function(e){
        ...
        var id      = $(this).data('id');
        var request = $.ajax({
          url:  '<?php echo base_url('index.php/action/all/get_print_invoice_done'); ?>',
          type: 'POST',
          data: {
            <?php echo $this->security->get_csrf_token_name(); ?> : '<?php echo $this->security->get_csrf_hash(); ?>',
            id : id,
      ...
      $(document).on('submit', '#form_print', function(e){
        ...
        if($('#form_print').valid() == true){
          ...
          var request   = $.ajax({
            url: '<?php echo base_url('index.php/action/all/print_invoice_done'); ?>',
      ...
      $(document).on('click', '.canceled', function(e){
        ...
        if(confirm("Anda yakin ingin menghapus faktur "+name+" ?")){
          show_loading_message();
          var request = $.ajax({
            url:  '<?php echo base_url('index.php/action/all/invoice_done_canceled'); ?>',
            type: 'POST',
            data: {
              <?php echo $this->security->get_csrf_token_name(); ?> : '<?php echo $this->security->get_csrf_hash(); ?>',
              id : id
