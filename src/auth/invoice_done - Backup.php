...
if($action != ''){
  ...
  if($action == 'sortdata_'.$slug){
    $query = "SELECT DISTINCT invoice_date FROM $tabel8 ORDER BY invoice_date DESC";
      ...
      while($row = $sql->fetch_array()){
        ...
        if($_SESSION['role'] != '5'){
          if(!in_array($row['no_invoice'], $data_invoice)){
            ...
            $functions .= '<li class="function_canceled"><a data-id="'.$row['id'].'" data-name="'.$row['no_invoice'].'" title="Batal"><span>Batal</span></a></li>';} 
        } else {
    ...
    $query = "SELECT a.id_fk, a.id_sj, a.no_delivery, a.send_qty, b.id, b.print, b.no_invoice, b.invoice_date, b.duration, b.input_by, b.note, c.price, c.unit, d.no_so, e.customer, e.po_customer, f.sj_date, f.cost, f.ekspedisi, f.uom, f.jml, g.ppn, i.name FROM delivery_orders_item AS a LEFT JOIN invoice AS b ON b.id_fk = a.id_fk AND b.id_sj = a.id_sj LEFT JOIN preorder_item AS c ON c.id_fk = a.id_fk AND c.item_to = a.item_to LEFT JOIN workorder_item AS d ON d.id_fk = a.id_fk AND d.item_to = a.item_to LEFT JOIN preorder_customer AS e ON e.id_fk = a.id_fk LEFT JOIN delivery_orders_customer AS f ON f.id_fk = a.id_fk AND f.id_sj = a.id_sj LEFT JOIN preorder_price AS g ON g.id_fk = a.id_fk LEFT JOIN status AS h ON h.id_fk = a.id_fk AND h.item_to = a.item_to LEFT JOIN user AS i ON i.id = b.input_by WHERE h.order_status BETWEEN 1 AND 2 AND b.status = 1 AND b.invoice_date BETWEEN '$dari' AND '$sampai' ORDER BY a.id ASC";
    $sql = $connect->query($query);
    if(!$sql){
      $result  = $error;
      $message = $qerror;
    ...
    if($id == ''){
      ...
      if(!$sql1 OR !$sql2){
        ...
        while($row = $sql2->fetch_array()){
          ...
          if($row['ppn'] > 0){ $ppn = $tagihan/10; $total = $tagihan + $ppn; } else { $ppn = 0; $total = $tagihan; }
          if(!in_array($row['id_fk'].'-'.$row['id_sj'].'-'.$row['cost'], $data_cost)){
            $biaya_kirim = $row['cost'];
          } else {
            $biaya_kirim = '';
    ...
    if($id == ''){
      ...
      foreach($data['price'] as $key => $value){
        ...
        if($status_ppn > 0){ $ppn = $price / 10; } else { $ppn = 0; }
      ...
      $update = "UPDATE $tabel8 SET print = '1' WHERE id = '$id'";
      $sql = $connect->query($update);
      $logger = logger($connect,'Print invoice (done)', 'Customer: '.$customer.' - Invoice: '.$no_invoice.' - Total: Rp '.$bill.' - BANK: '.$bank);
    ...
    if($id == ''){
      ...
      $update = "UPDATE $tabel8 SET status = '0', complete_date = '', note = '' WHERE id = '".$id."'";
      $sql2 = $connect->query($update);

      $logger = logger($connect,'Canceled invoice', ' No invoice: '.$fetch['no_invoice'].' - Invoice date: '.$fetch['invoice_date'].' - Customer: '.$fetch['customer'].' - No delivery: '.$fetch['no_delivery']);
